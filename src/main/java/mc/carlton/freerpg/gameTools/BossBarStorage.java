...
public class BossBarStorage {
    ...
    public void initializeNewPlayer(Player p){
        ...
        BossBar expBar = Bukkit.createBossBar(new NamespacedKey(plugin,p.getUniqueId().toString()),p.getDisplayName()+"'s EXP Bar", BarColor.GREEN, BarStyle.SOLID);
        expBar.setVisible(false);
        expBar.addPlayer(p);
        BossBar abilityDurationBar1 = Bukkit.createBossBar(new NamespacedKey(plugin,p.getUniqueId().toString()),p.getDisplayName()+"'s Ability Duration Bar 1", BarColor.YELLOW, BarStyle.SOLID);
        abilityDurationBar1.setVisible(false);
        abilityDurationBar1.addPlayer(p);
        BossBar abilityDurationBar2 = Bukkit.createBossBar(new NamespacedKey(plugin,p.getUniqueId().toString()),p.getDisplayName()+"'s Ability Duration Bar 1", BarColor.YELLOW, BarStyle.SOLID);
        abilityDurationBar2.setVisible(false);
        abilityDurationBar2.addPlayer(p);
        BossBar abilityDurationBar3 = Bukkit.createBossBar(new NamespacedKey(plugin,p.getUniqueId().toString()),p.getDisplayName()+"'s Ability Duration Bar 1", BarColor.YELLOW, BarStyle.SOLID);
