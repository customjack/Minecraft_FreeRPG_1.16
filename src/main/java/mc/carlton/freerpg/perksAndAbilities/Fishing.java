...
public class Fishing extends Skill {
      ...
      if (cooldown < 1) {
        ...
        if (prepMessages > 0) {
          actionMessage.sendMessage(
              ChatColor.GRAY + ">>>" + lang.getString("prepare") + " " + lang.getString(
                  "fishingRod") + "...<<<");
        }
        ...
        actionMessage.sendMessage(
            ChatColor.RED + lang.getString("superBait") + " " + lang.getString("cooldown") + ": "
  ...
  public void enableAbility() {
    ...
    Map<String, ArrayList<Number>> pStat = pStatClass.getPlayerData();
    actionMessage.sendMessage(
        ChatColor.GREEN + ChatColor.BOLD.toString() + ">>>" + lang.getString("superBait") + " "
    ...
    String coolDownEndMessage =
        ChatColor.GREEN + ">>>" + lang.getString("superBait") + " " + lang.getString("readyToUse")
            + "<<<";
    String endMessage =
        ChatColor.RED + ChatColor.BOLD.toString() + ">>>" + lang.getString("superBait") + " "
  ...
  public void grapplingHook(FishHook fishhook, World world) {
    ...
    Vector velocity = new Vector(dx * multiplier,
        (dy * multiplier) + (double) Math.sqrt(distance) * 0.10, dz * multiplier);
    ...
    if (hookedEntity == null) {
      ...
      if (world.getBlockAt(location).getType() == Material.WATER) {
        if (!superBaitBlock.contains(p.getUniqueId())) {
          ...
          Vector velocity = new Vector(dx * multiplier,
              (dy * multiplier) + (double) Math.sqrt(distance) * 0.1, dz * multiplier);
          droppedItem.setVelocity(velocity);
          ((ExperienceOrb) world.spawn(p.getLocation(), ExperienceOrb.class)).setExperience(
              rand.nextInt(6) + 1);
                ...
                public void run() {
                  ...
                  if (count == 20) {
                    ...
                    if (addEffect) {
                      ...
                      p.addPotionEffect(
                          new PotionEffect(PotionEffectType.DOLPHINS_GRACE, 15 * 20, 0));
                    }
                    ...
                    if (addEffect2) {
                      ...
                      p.addPotionEffect(
                          new PotionEffect(PotionEffectType.NIGHT_VISION, 15 * 20, 0));
                    }
                  }
                }
  ...
  public void eatFishFood(ItemStack food) {
    ...
    if (fishFood.containsKey(food.getType())) {
      ...
      p.setFoodLevel((int) Math.min(20,
          p.getFoodLevel() + Math.round(foodMultiplier * fishFood.get(foodType))));
      p.setSaturation((float) Math.min(p.getFoodLevel(),
          p.getSaturation() + (foodMultiplier * fishFoodSaturation.get(foodType))));
    }
  ...
  public void rob(FishHook fishhook, Entity hookedEntity, World world) {
    ...
    if (pTimers[11] > 0) {
      actionMessage.sendMessage(
          ChatColor.RED + lang.getString("rob") + " " + lang.getString("cooldown") + ": "
              + pTimers[11].toString() + "s");
      return;
    }
    Map<String, ArrayList<Number>> pStat = pStatClass.getPlayerData();
  ...
  public ItemStack dropTable(boolean superBaitOn) {
    ...
    double[] tierChances = {Math.max(0.100 - 0.02 * luckOfTheSeaLevel, 0),
        Math.min(0.050 + 0.2 * luckOfTheSeaLevel, 0.15),
        baseChances.get(0), baseChances.get(1), baseChances.get(2), baseChances.get(3),
        baseChances.get(4)};
    ...
    Material[] music_discs = {Material.MUSIC_DISC_11, Material.MUSIC_DISC_13,
        Material.MUSIC_DISC_BLOCKS, Material.MUSIC_DISC_CAT,
    ...
    Material[] chainmail = {Material.CHAINMAIL_BOOTS, Material.CHAINMAIL_CHESTPLATE,
    ...
    if (roll < rollBrackets[0]) { //Junk Tier (-1)
      ...
      double[] naturalJunkBrackets = {0.12, 0.144, 0.264, 0.384, 0.504, 0.564, 0.624, 0.744, 0.864,
          0.878, 1}; //Altered a bit due to rounding error
      ...
      ItemStack[] junkItems = {new ItemStack(Material.BOWL, 1), junkRod,
          new ItemStack(Material.LEATHER, 1), junkBoots,
          new ItemStack(Material.ROTTEN_FLESH, 1), new ItemStack(Material.STICK, 1),
          new ItemStack(Material.STRING, 1),
          new ItemStack(Material.GLASS_BOTTLE, 1), new ItemStack(Material.BONE, 1), junkInk,
      ...
      for (int i = 0; i < naturalJunkBrackets.length; i++) {
        if (roll2 < naturalJunkBrackets[i]) {
          increaseStats.changeEXP(skillName,
              (int) Math.round(expMultiplier * expMap.get("catchJunk")));
          return junkItems[i];
        }
      }
    } else if (roll < rollBrackets[1]) { //Natural Tier (0)
      double roll2 = rand.nextDouble();
      double[] naturalRareBrackets = {0.143, 0.283, 0.429, 0.572, 0.715, 0.858,
          1}; //Altered a bit due to rounding error
      ...
      ItemStack[] items = {rareBow, rareRod, enchantedBook, new ItemStack(Material.NAME_TAG, 1),
          new ItemStack(Material.NAUTILUS_SHELL, 1), new ItemStack(Material.SADDLE, 1),
          new ItemStack(Material.LILY_PAD, 1)};
      for (int i = 0; i < naturalRareBrackets.length; i++) {
        if (roll2 < naturalRareBrackets[i]) {
          increaseStats.changeEXP(skillName,
              (int) Math.round(expMultiplier * expMap.get("catchTier0")));
          return items[i];
        }
      }
      ...
      if (roll2 < 0.2) {
        if (itemNames.get(0) == null) {
          ...
          if (hotRod > 0) {
            ...
            item.setAmount(
                dropAmounts_hotRod.get(0) + rand.nextInt(dropRandomness_hotRod.get(0) + 1));
          } else {
        ...
        if (itemNames.get(1) == null) {
          ...
          if (hotRod > 0) {
            ...
            item.setAmount(
                dropAmounts_hotRod.get(1) + rand.nextInt(dropRandomness_hotRod.get(1) + 1));
          } else {
      ...
      increaseStats.changeEXP(skillName,
          (int) Math.round(expMultiplier * expMap.get("catchTier1")));
      ...
      if (roll2 < 0.2) {
        if (itemNames.get(4) == null) {
          ...
          if (hotRod > 0) {
            ...
            item.setAmount(
                dropAmounts_hotRod.get(2) + rand.nextInt(dropRandomness_hotRod.get(2) + 1));
          } else {
        ...
        if (itemNames.get(6) == null) {
          ...
          if (hotRod > 0) {
            ...
            item.setAmount(
                dropAmounts_hotRod.get(3) + rand.nextInt(dropRandomness_hotRod.get(3) + 1));
          } else {
        ...
        if (itemNames.get(7) == null) {
          ...
          if (hotRod > 0) {
            ...
            item.setAmount(
                dropAmounts_hotRod.get(4) + rand.nextInt(dropRandomness_hotRod.get(4) + 1));
          } else {
      ...
      increaseStats.changeEXP(skillName,
          (int) Math.round(expMultiplier * expMap.get("catchTier2")));
      ...
      increaseStats.changeEXP(skillName,
          (int) Math.round(expMultiplier * expMap.get("catchTier3")));
      ...
      increaseStats.changeEXP(skillName,
          (int) Math.round(expMultiplier * expMap.get("catchTier4")));
      ...
      increaseStats.changeEXP(skillName,
          (int) Math.round(expMultiplier * expMap.get("catchTier5")));
    } else { //fish roll
      double roll2 = rand.nextDouble();
      if (roll2 < .6) {
  ...
  public ItemStack getTieredLoot(int tier) {
    ...
    Material[] tools0 = {Material.DIAMOND_PICKAXE, Material.IRON_PICKAXE, Material.DIAMOND_SHOVEL,
        Material.IRON_SHOVEL, Material.DIAMOND_AXE, Material.IRON_AXE};
    ...
    if (dropMeta instanceof Damageable) {
      ((Damageable) dropMeta).setDamage(
          (int) Math.round(drop.getType().getMaxDurability() * (0.85 * rand.nextDouble()) + 1));
    }
