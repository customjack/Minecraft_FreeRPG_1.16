...
public class Smelting extends Skill {
    ...
    new BukkitRunnable() {
      ...
      public void run() {
        if (checkIfBlockIsFurnace(world, furnaceLoc)) {
          ...
          FurnaceOneTickLater.setCookTimeTotal(
              (int) Math.round(finalDefaultCookTime / speedUpFactor));
    ...
    new BukkitRunnable() {
      ...
      public void run() {
        if (checkIfBlockIsFurnace(world, furnaceLoc)) {
          ...
          runningFurnace.setBurnTime(
              (short) Math.ceil(3 + (burnLengthMultiplier * defaultBurnTime / speedUpFactor)));
  ...
  public boolean flamePick(Block block, World world, Material blockType, boolean giveEXP) {
    ...
    world.dropItemNaturally(block.getLocation(),
        new ItemStack(itemGroups.getSmeltableItemsMap().get(blockType), dropAmount));
    return true;
