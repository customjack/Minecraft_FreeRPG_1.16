...
public class YMLManager {
  ...
  public void updateCheckYML(String fileName) {
    ...
    if (fileName.equalsIgnoreCase("config.yml") || fileName.equalsIgnoreCase("advancedConfig.yml")
        || fileName.equalsIgnoreCase("perkConfig.yml")) {
  ...
  public void storeOldFile(String fileName) {
    ...
    f.setReadable(true, false);
    f.setWritable(true, false);
    File oldLanguagesYMLFolder = new File(plugin.getDataFolder(),
        File.separator + "OutdatedYMLFiles");
  ...
  public void updateLanguagesYML(String fileName) {
    Plugin plugin = FreeRPG.getPlugin(FreeRPG.class);
    FreeRPG.log(Level.INFO, "[FreeRPG] " + fileName
        + " keys mismatch the current version's keys. The file may be updated... ");
    ...
    for (String key : oldYAML.getConfigurationSection("lang")
        .getKeys(false)) { //Checks all languages in the old file
      if (!newYAML.contains("lang." + key) || key.equalsIgnoreCase(
          "custom")) { //Checks for new languages in the old file
        ...
        for (String lastLevelKey : newYAML.getConfigurationSection("lang.enUs")
            .getKeys(false)) { //For every key...
          String customKey = "lang." + key + "." + lastLevelKey;
          String enUsKey = "lang.enUs." + lastLevelKey;
          if (oldYAML.contains(customKey)) {
            newYAML.set(customKey,
                oldYAML.get(customKey)); //Add the custom language's key to the new file
          } else {
            newYAML.set(customKey,
                newYAML.get(enUsKey)); //Set the custom language's missing key to the new file
            addedLines = true;
            needToSave = true;
          }
        }
      } else { //Checks other languages to see if they were changed, overwrites any changes
        for (String lastLevelKey : newYAML.getConfigurationSection("lang." + key)
            .getKeys(false)) { //for all language keys
          String fullKey = "lang." + key + "." + lastLevelKey;
          if (oldYAML.contains(fullKey)) { //if the old file has this key
            if (!(oldYAML.get(fullKey).equals(
                newYAML.get(fullKey)))) { //and that key is different from the resource file's key
  ...
  public void updateYML(String fileName) {
    Plugin plugin = FreeRPG.getPlugin(FreeRPG.class);
    FreeRPG.log(Level.INFO,
        "[FreeRPG] " + fileName + " keys mismatch the current version's keys. " +
    ...
    for (String key : lastLevelKeys) {
      if (oldYAML.contains(key) && newYAML.contains(key)) {
        if (!oldYAML.get(key).equals(newYAML.get(key))) {
          newYAML.set(key,
              oldYAML.get(key)); //Sets the new config to whatever data was in the old config
          changeMade = true;
        }
      }
    }
    ...
    if (changeMade) {
      ...
      FreeRPG.log(Level.INFO,
          "[FreeRPG] " + fileName + " updated to default version successfully! " +
