...
public class Defense extends Skill {
      ...
      if (cooldown < 1) {
        ...
        if (prepMessages > 0) {
          actionMessage.sendMessage(
              ChatColor.GRAY + ">>>" + lang.getString("prepare") + " " + lang.getString("yourself")
                  + "...<<<");
        }
        ...
        actionMessage.sendMessage(
            ChatColor.RED + lang.getString("stoneSoldier") + " " + lang.getString("cooldown") + ": "
  ...
  public void enableAbility() {
    ...
    Map<String, ArrayList<Number>> pStat = pStatClass.getPlayerData();
    actionMessage.sendMessage(
        ChatColor.GREEN + ChatColor.BOLD.toString() + ">>>" + lang.getString("stoneSoldier") + " "
    ...
    String coolDownEndMessage =
        ChatColor.GREEN + ">>>" + lang.getString("stoneSoldier") + " " + lang.getString(
            "readyToUse") + "<<<";
    String endMessage =
        ChatColor.RED + ChatColor.BOLD.toString() + ">>>" + lang.getString("stoneSoldier") + " "
    ...
    if (pAbilities[8] == -2) {
      ...
      for (PotionEffect effect : p.getActivePotionEffects()) {
        if (effect.getType().equals(PotionEffectType.DAMAGE_RESISTANCE)
            && effect.getDuration() <= duration && effect.getAmplifier() == 2) {
          p.removePotionEffect(PotionEffectType.DAMAGE_RESISTANCE);
        } else if (effect.getType().equals(PotionEffectType.SLOW)
            && effect.getDuration() <= duration && effect.getAmplifier() == slowBuff) {
          p.removePotionEffect(PotionEffectType.SLOW);
    ...
    for (PotionEffect effect : p.getActivePotionEffects()) {
      if (effect.getType().equals(PotionEffectType.DAMAGE_RESISTANCE)) {
        ...
        if ((effect.getDuration() > duration && effect.getAmplifier() >= buffLevel) || (
            effect.getAmplifier() > buffLevel && effect.getDuration() > 40)) {
          addEffect = false;
        }
  ...
  public void hearty() {
    ...
    if (!runMethods) {
      toggleHearty(0,
          heartyToggle); //Toggle hearty with level not high enough to be active (can only be turned off if it was on)
      return;
    }
  ...
  public void giveHitEXP(double damage, Entity entity) {
    ...
    double expToReward =
        (damage * expMap.get("takeDamage_EXPperDamagePointDone") + expMap.get("takeDamage"))
            * multiplier;
