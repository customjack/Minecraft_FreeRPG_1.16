...
public class CustomItem extends CustomContainer {
  ...
  protected ItemStack getItemStackWithoutPotionEffects() {
    ...
    if (itemMeta instanceof Damageable) {
      ...
      double randomDamageMultiplier =
          1.0 - (random.nextDouble() * (maxDurabilityPortion - minDurabilityPortion)
              + minDurabilityPortion);
    ...
    int enchantLevel =
        random.nextInt((maxEnchantmentLevel + 1) - minEnchantmentLevel) + minEnchantmentLevel;
    if (enchantLevel > 0) {
   ...
   *
   * @param minDurabilityPortion minimum durability portion (relative to it's full durability) the
   *                             item can be set to (<= 1)
   */
   ...
   *
   * @param maxDurabilityPortion maximum durability portion (relative to it's full durability) the
   *                             item can be set to (<=1)
   */
  ...
  /**
   * Sets the drop probability weight for this item. This value is compared to other weights of
   * items in a drop table
   *
