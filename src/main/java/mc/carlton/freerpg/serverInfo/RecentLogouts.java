...
public class RecentLogouts {
    ...
    public void playerLogout(Player p,boolean disablePlugin) {
        ...
        if (lastLogouts.contains(playerUUID)) { //If a player logs out while they were already in the list of recent logouts, remove them
            lastLogouts.remove(playerUUID);
        }
        removeLastEntries(disablePlugin); //Ensure the recent logout list never exceeds a specified limit (maxPlayerStored)
    ...
    public void removeLastEntries(boolean disablePlugin) {
        ...
        if (playersStored >= maxPlayersStored ) { //We are about to add a person, so if the list is full we need to remove the last person on this list
            ...
            for (int i = 0; i <= (playersStored-maxPlayersStored); i++) {
                ...
                if (!disablePlugin) { //They will be removed anyway if the plugin is disabling
                    offlinePlayerStatLoadIn.unloadOfflinePlayer(removedPlayerUUID); //Unloads the players stats ONLY if they're offline
                }
