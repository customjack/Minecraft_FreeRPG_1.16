...
import mc.carlton.freerpg.skillAndPerkInfo.*;
...
public class PerkConfig {
    ...
    private static final List<String> DROP_TABLE_KEYS            = Arrays.asList(new String[]{"dropTable", "dropsAdded"});
    private static final List<String> MOB_DROP_TABLE_KEYS        = Arrays.asList(new String[]{"mobDropTable", "entityDropTable"});
    private static final List<String> MOB_LIST_KEYS              = Arrays.asList(new String[]{"mobList","entityList","entityTypeList"});
    private static final List<String> MATERIAL_LIST_KEYS         = Arrays.asList(new String[]{"itemList","blockList"});
    private static final List<String> DAMAGE_LIST_KEYS           = Arrays.asList(new String[]{"damageTypeList","damageCauseList"});
    private static final List<String> CUSTOM_RECIPE_LIST_KEYS    = Arrays.asList(new String[]{"recipes","customRecipes"});
    private static final List<String> CUSTOM_DROP_MAP_KEYS       = Arrays.asList(new String[]{"dropChanges","changedDrops"});
    private static final List<String> POTION_DATA_LIST_KEYS      = Arrays.asList(new String[]{"potionDataList","potionTypeList"});
    private static final List<String> CUSTOM_POTION_KEYS         = Arrays.asList(new String[]{"decreasingTiers","increasingTiers"});
    private static final List<String> CUSTOM_EFFECTS_GIVEN_KEYS  = Arrays.asList(new String[]{"effectsGiven","customEffect"});
    private static final List<String> SPECIAL_KEYS               = Arrays.asList(new String[]{"decreasingTiers","increasingTiers","affectedMobs","added-enchantmentList","changedEffects"});
    private static final List<String> IGNORED_KEYS               = Arrays.asList(new String[]{"effectChance","activationChance","enabled", "startingValue", "allowedRange", "changePerLevel"});
    ...
    static public void readInAllPerkInfo(FileConfiguration config) {
        for (String skillNameString: config.getKeys(false)) {
            ...
            if (skillName != null) {
                allPerkInfo.put(skillName,getSkillTreeInfoFromConfig(skillName, config.getConfigurationSection(skillNameString)));
            }
        }
    }
    ...
    static private SkillTreeInfo getSkillTreeInfoFromConfig(SkillName skillName, ConfigurationSection configSection) {
        ...
        for (String skillIdString: configSection.getKeys(false)) {
            skillTreeInfo.addPerkInfo(skillIdString,getSkillPerkInfoFromConfig(configSection.getConfigurationSection(skillIdString)));
        }
    ...
    static private PerkInfo getSkillPerkInfoFromConfig(ConfigurationSection configSection) {
        if (configSection.getName().contains("passive")) { //Passive Skill
            ...
            for (String key : configSection.getKeys(false)) { //Get each level information, also determine min and max levels
                String[] splitKey = key.split("-");
                if (splitKey.length == 2 && splitKey[0].equalsIgnoreCase("level")) { //Level information
                    int level = Integer.valueOf(splitKey[1]);
                    skillPerkInfo.addSkillPerkInfo(level,getSkillPerkLevelFromConfig(configSection.getConfigurationSection(key),level)); //Adds the skillPerkInfo
                    if (level < minLevel) {
                        minLevel = level;
                    }
                    if (level > maxLevel) {
    ...
    static private void addNodeInformation(LowestLevelInfo lowLevelInformationHolder, ConfigurationSection configurationSection, String key) {
    ...
    static private SkillPerkLevelInfo getSkillPerkLevelFromConfig(ConfigurationSection configSection, int level) {
    ...
    static private Object getNodeInformation(ConfigurationSection configSection, String localConfigNodeName) {
        ...
        } else if (UtilityMethods.stringContainsIgnoreCase(localConfigNodeName,CUSTOM_RECIPE_LIST_KEYS)) {
